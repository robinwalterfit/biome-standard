...
1. Before you start your work, make sure to set up the repository
...
2. This repository uses [Conventional Commits](https://www.conventionalcommits.org/en/v1.0.0/).
   Make sure to follow the pattern. Release management is handled with the help
   of [Cocogitto](https://docs.cocogitto.io/). Cocogitto can be used to create
   commits, too. Refer to `cog commit --help` or the official documentation if
   you want to learn more.
   Make sure to update the `authors` attribute under the `changelog` table in
   [`cog.toml`](./cog.toml).
...
version number will be calculated by `cocogitto` based on the commit messages.
